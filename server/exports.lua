...
local function AddJob(jobName, job)
    ...
    TriggerClientEvent('QBCore:Client:OnSharedUpdate', -1,'الوظائف', jobName, job)
...
local function AddJobs(jobs)
    ...
    TriggerClientEvent('QBCore:Client:OnSharedUpdateMultiple', -1, 'الوظائف', jobs)
    TriggerEvent('QBCore:Server:UpdateObject')
...
local function AddItem(itemName, item)
    ...
    TriggerClientEvent('QBCore:Client:OnSharedUpdate', -1, 'العناصر', itemName, item)
    TriggerEvent('QBCore:Server:UpdateObject')
    return true, "success"
end
...
local function AddItems(items)
    ...
    if not shouldContinue then return false, message, errorItem end
    TriggerClientEvent('QBCore:Client:OnSharedUpdateMultiple', -1, 'العناصر', items)
    TriggerEvent('QBCore:Server:UpdateObject')
    return true, message, nil
end
...
local function AddGang(gangName, gang)
    ...
    TriggerClientEvent('QBCore:Client:OnSharedUpdate', -1, 'العصابات', gangName, gang)
    TriggerEvent('QBCore:Server:UpdateObject')
    return true, "success"
end
...
local function AddGangs(gangs)
    ...
    if not shouldContinue then return false, message, errorItem end
    TriggerClientEvent('QBCore:Client:OnSharedUpdateMultiple', -1, 'العصابات', gangs)
    TriggerEvent('QBCore:Server:UpdateObject')
    return true, message, nil
end
...
local function GetCoreVersion(InvokingResource)
    ...
    if InvokingResource and InvokingResource ~= '' then
        print(("%s called qbcore version check: %s"):format(InvokingResource or 'مصدر غير معروف', resourceVersion))
    end
